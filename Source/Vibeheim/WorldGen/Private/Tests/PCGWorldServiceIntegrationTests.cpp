...
public:
        ...
        FBox LastExecutionBounds = FBox(ForceInitToZero);
        int32 LastSeed = 0;
...
private:
        void BuildSuccessfulOutput()
        {
                ...
                auto EnsureAttribute = [Metadata, EntryKey](const FName& AttributeName, auto&& Value)
                {
                        ...
                        if constexpr (std::is_same_v<ValueType, FGuid>)
                        {
                                using StoredType = FString;
                                FPCGMetadataAttribute<StoredType>* Attribute = Metadata->GetMutableTypedAttribute<StoredType>(AttributeName);
                                if (!Attribute)
                                {
                                        Attribute = Metadata->CreateAttribute<StoredType>(AttributeName, FString(), false, true);
                                }
                                if (Attribute)
                                {
                                        Attribute->SetValue(EntryKey, Value.ToString(EGuidFormats::DigitsWithHyphens));
                                }
                        }
                        else
                        {
...
bool FPCGWorldServiceSchedulerSuccessTest::RunTest(const FString& Parameters)
{
        ...
        FPCGWorldServiceTestAccessor::SetScheduler(Service, FPCGSchedulerExecutorPtr(Executor.Release()));
