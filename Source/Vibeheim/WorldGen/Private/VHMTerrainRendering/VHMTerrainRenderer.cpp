...
UVHMTerrainRenderer::UVHMTerrainRenderer() {
  ...
  CachedWorld = nullptr;
  PrebakedTerrainResource = nullptr;
...
bool UVHMTerrainRenderer::Initialize(
    UWorldGenSettings *Settings, UHeightfieldService *InHeightfieldService,
    UTileStreamingService *InTileStreamingService) {
  if (!Settings) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHMTerrainRenderer::Initialize - Invalid Settings reference"));
    return false;
  }
  ...
  if (bServicesRequired && (!InHeightfieldService || !InTileStreamingService)) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHMTerrainRenderer::Initialize - Invalid service references "
  ...
  if (!CachedWorld) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHMTerrainRenderer::Initialize - No valid world context"));
    return false;
  }
  ...
  if (!InitializeHeightfieldTextureManager()) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHMTerrainRenderer::Initialize - Failed to initialize "
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("VHMTerrainRenderer initialized successfully"));
  return true;
}

bool UVHMTerrainRenderer::InitializeWithBiomeService(
    UWorldGenSettings *Settings, UHeightfieldService *InHeightfieldService,
    UTileStreamingService *InTileStreamingService,
  ...
  if (!InBiomeService) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeWithBiomeService - BiomeService is null"));
    return false;
  }
  ...
  if (!InitializeTerrainMaterialSystem()) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeWithBiomeService - Failed to initialize "
                "TerrainMaterialSystem"));
    return false;
  }

  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("VHMTerrainRenderer initialized with BiomeService successfully"));
  return true;
}

bool UVHMTerrainRenderer::CreateTerrainMeshForTile(
    const FTileCoord &TileCoord) {
  if (!IsValidTileCoordinate(TileCoord)) {
    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("CreateTerrainMeshForTile - Invalid tile coordinate: (%d, %d)"),
  ...
  if (TerrainMeshes.Contains(TileCoord)) {
    UE_LOG(LogVHMTerrainRenderer, Log,
           TEXT("CreateTerrainMeshForTile - Mesh already exists for tile: (%d, "
                "%d)"),
           TileCoord.X, TileCoord.Y);
    return true;
  }
  ...
  if (!bUsingPrebaked) {
    ...
    HeightTexture = TextureManagerInterface
                        ? TextureManagerInterface->CreateHeightTexture(
                              TileCoord, ProcessedHeightData)
                        : nullptr;
  }

  if (!HeightTexture) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHM001: Failed to create height texture for tile (%d, %d) - "
                "using flat meadow fallback"),
           TileCoord.X, TileCoord.Y);
    return CreateFlatMeadowFallback(TileCoord);
  }
  ...
  UVirtualHeightfieldMeshComponent *VHMComponent =
      CreateVHMComponent(TileCoord);
  if (!VHMComponent) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("VHM001: Failed to create VHM component for tile (%d, %d) - "
                "using flat meadow fallback"),
           TileCoord.X, TileCoord.Y);
    return CreateFlatMeadowFallback(TileCoord);
  }
  ...
  if (!GenerateMeshFromHeightfield(VHMComponent, TileCoord, HeightTexture)) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("CreateTerrainMeshForTile - Failed to generate mesh from "
                "heightfield for tile: (%d, %d)"),
           TileCoord.X, TileCoord.Y);
  ...
  if (!ValidateVHMComponent(VHMComponent, TileCoord)) {
    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("CreateTerrainMeshForTile - VHM component validation failed "
                "for tile: (%d, %d)"),
           TileCoord.X, TileCoord.Y);
  ...
  if (BiomeService && TerrainMaterialSystem.GetInterface()) {
    ...
    if (BiomeService->GetBiomeDefinition(TileBiome, BiomeDefinition)) {
      ...
      if (TileMaterial) {
        ...
        // Use explicit base class call to avoid protected SetMaterial() in
        // UE5.6
        ...
        if (VHMComponent->UPrimitiveComponent::GetMaterial(0) == TileMaterial) {
          UE_LOG(LogVHMTerrainRenderer, Verbose,
                 TEXT("Applied material for biome %s to tile (%d, %d)"),
                 *BiomeDefinition.BiomeName, TileCoord.X, TileCoord.Y);
        } else {
          UE_LOG(LogVHMTerrainRenderer, Warning,
                 TEXT("Failed to apply material for biome %s to tile (%d, %d) "
  ...
  if (ITerrainLODManager *LODManagerInterface =
          Cast<ITerrainLODManager>(TerrainLODManager.GetObject())) {
    if (UVHMTerrainLODManager *ConcreteLODManager =
            Cast<UVHMTerrainLODManager>(TerrainLODManager.GetObject())) {
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Created terrain mesh for tile (%d, %d) in %.2fms"), TileCoord.X,
         TileCoord.Y, GenerationTime);
  return true;
}

bool UVHMTerrainRenderer::UpdateTerrainMesh(
    const FTileCoord &TileCoord,
    const TArray<FHeightfieldModification> &Modifications) {
  if (!TerrainMeshes.Contains(TileCoord)) {
    // Create on demand for edited tiles to ensure visual + collision update
    // within target
    if (!CreateTerrainMeshForTile(TileCoord)) {
      UE_LOG(LogVHMTerrainRenderer, Warning,
             TEXT("UpdateTerrainMesh - No mesh exists and creation failed for "
                  "tile: (%d, %d)"),
             TileCoord.X, TileCoord.Y);
      return false;
    }
  ...
  if (!VHMSettings.bEnableRealTimeEditing) {
    UE_LOG(LogVHMTerrainRenderer, Log,
           TEXT("UpdateTerrainMesh - Real-time editing disabled, skipping "
                "update for tile: (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return true;
  }
  ...
  IHeightfieldTextureManager *TextureManagerInterface =
      Cast<IHeightfieldTextureManager>(HeightfieldTextureManager.GetObject());
  bool bTextureUpdated = TextureManagerInterface
                             ? TextureManagerInterface->UpdateHeightTexture(
                                   TileCoord, Modifications)
                             : false;
  if (!bTextureUpdated) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("UpdateTerrainMesh - Failed to update height texture for tile: "
                "(%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (MeshData.VHMComponent && MeshData.HeightTexture) {
    if (!GenerateMeshFromHeightfield(MeshData.VHMComponent, TileCoord,
                                     MeshData.HeightTexture)) {
      UE_LOG(LogVHMTerrainRenderer, Error,
             TEXT("UpdateTerrainMesh - Failed to regenerate mesh for tile (%d, "
                  "%d)"),
             TileCoord.X, TileCoord.Y);
      return false;
    }
    ...
    UE_LOG(LogVHMTerrainRenderer, Log,
           TEXT("Regenerated VHM mesh for tile (%d, %d) after height texture "
                "update"),
           TileCoord.X, TileCoord.Y);
  }
  ...
  if (BiomeService && TerrainMaterialSystem.GetInterface()) {
    ...
    if (HeightfieldService->GetCachedHeightfield(TileCoord,
                                                 UpdatedHeightfieldData)) {
      // Determine biome for updated terrain
      EBiomeType TileBiome = BiomeService->DetermineTileBiome(
          TileCoord, UpdatedHeightfieldData.HeightData);
      FBiomeDefinition BiomeDefinition;
      if (BiomeService->GetBiomeDefinition(TileBiome, BiomeDefinition)) {
        // Update material parameters
        TerrainMaterialSystem.GetInterface()->UpdateMaterialParameters(
            TileCoord, BiomeDefinition);
        UE_LOG(LogVHMTerrainRenderer, Verbose,
               TEXT("Updated material parameters for tile (%d, %d)"),
               TileCoord.X, TileCoord.Y);
      }
    }
  }
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Updated terrain mesh for tile (%d, %d) in %.2fms"), TileCoord.X,
         TileCoord.Y, UpdateTime);
  return true;
}
...
void UVHMTerrainRenderer::RemoveTerrainMesh(const FTileCoord &TileCoord) {
  ...
  if (IHeightfieldTextureManager *TextureManagerInterface =
          Cast<IHeightfieldTextureManager>(
              HeightfieldTextureManager.GetObject())) {
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Removed terrain mesh for tile: (%d, %d)"), TileCoord.X,
         TileCoord.Y);
}

UVirtualHeightfieldMeshComponent *
UVHMTerrainRenderer::GetVHMComponent(const FTileCoord &TileCoord) {
...
FVHMPerformanceStats UVHMTerrainRenderer::GetPerformanceStats() const {
  ...
  IHeightfieldTextureManager *TextureManagerInterface =
      Cast<IHeightfieldTextureManager>(HeightfieldTextureManager.GetObject());
  PerformanceStats.TextureMemoryUsageMB =
      TextureManagerInterface
          ? TextureManagerInterface->GetTextureMemoryUsageMB()
          : 0.0f;
  ...
  if (RecentMeshGenerationTimes.Num() > 0) {
    ...
    PerformanceStats.AverageMeshGenerationMs =
        TotalTime / RecentMeshGenerationTimes.Num();
  }
  ...
  if (ITerrainLODManager *LODManagerInterface =
    ...
    PerformanceStats.LODTransitionsThisFrame =
        LODManagerInterface->GetLODTransitionsThisFrame();
  }

  return PerformanceStats;
}

void UVHMTerrainRenderer::OnTileStreamingEvent(const FTileCoord &TileCoord,
                                               bool bTileLoaded) {
  if (bTileLoaded) {
    // Tile was loaded - check if it should be visible before creating mesh
    if (!TerrainMeshes.Contains(TileCoord)) {
      // Use LOD manager to determine if tile should be created based on
      // visibility
      ...
      if (ITerrainLODManager *LODManagerInterface =
        ...
        if (CachedWorld && CachedWorld->GetFirstPlayerController()) {
          if (APawn *PlayerPawn =
                  CachedWorld->GetFirstPlayerController()->GetPawn()) {
            ViewerPosition = PlayerPawn->GetActorLocation();
          }
        }

        bShouldCreateMesh =
            LODManagerInterface->IsTileVisible(TileCoord, ViewerPosition);
      }

      if (bShouldCreateMesh) {
        CreateTerrainMeshForTile(TileCoord);
      } else {
        UE_LOG(LogVHMTerrainRenderer, VeryVerbose,
               TEXT("Skipping mesh creation for tile (%d, %d) - not visible"),
               TileCoord.X, TileCoord.Y);
      }
    }
  } else {
...
void UVHMTerrainRenderer::Cleanup() {
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("VHMTerrainRenderer cleanup completed"));
}

void UVHMTerrainRenderer::SetPrebakedTerrainResource(
    UWorldGenTerrainResource *InResource) {
  PrebakedTerrainResource = InResource;
  if (PrebakedTerrainResource) {
    UE_LOG(
        LogVHMTerrainRenderer, Log,
        TEXT(
            "VHMTerrainRenderer: Set PrebakedTerrainResource with %d textures"),
        PrebakedTerrainResource->HeightTextures.Num());
  }
}
...
bool UVHMTerrainRenderer::GetTerrainMeshData(
    const FTileCoord &TileCoord, FTerrainMeshData &OutMeshData) const {
  if (const FTerrainMeshData *MeshData = TerrainMeshes.Find(TileCoord)) {
...
UVirtualHeightfieldMeshComponent *
UVHMTerrainRenderer::CreateVHMComponent(const FTileCoord &TileCoord) {
  if (!CachedWorld) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("CreateVHMComponent - No valid world context"));
    return nullptr;
  }
  ...
  if (TerrainMeshes.Contains(TileCoord)) {
    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("CreateVHMComponent - Tile (%d, %d) already has a mesh, "
                "cleaning up first"),
           TileCoord.X, TileCoord.Y);
    CleanupMeshData(TileCoord);
    TerrainMeshes.Remove(TileCoord);
  ...
  AActor *VHMActor =
      CachedWorld->SpawnActor<AActor>(AActor::StaticClass(), SpawnParams);
  if (!VHMActor) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("CreateVHMComponent - Failed to spawn VHM actor for tile: (%d, "
                "%d)"),
           TileCoord.X, TileCoord.Y);
    return nullptr;
  }
  ...
  VHMActor->SetActorLabel(
      FString::Printf(TEXT("VHMTerrain_Tile_%d_%d"), TileCoord.X, TileCoord.Y));
#endif

  // Create and attach VHM component
  UVirtualHeightfieldMeshComponent *VHMComponent =
      NewObject<UVirtualHeightfieldMeshComponent>(VHMActor);
  if (!VHMComponent) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("CreateVHMComponent - Failed to create VHM component for tile: "
                "(%d, %d)"),
           TileCoord.X, TileCoord.Y);
    VHMActor->Destroy();
    return nullptr;
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Created VHM component for tile (%d, %d) at position (%s)"),
         TileCoord.X, TileCoord.Y, *TileWorldPosition.ToString());

  return VHMComponent;
}

void UVHMTerrainRenderer::ConfigureVHMComponent(
    UVirtualHeightfieldMeshComponent *VHMComponent,
    const FTileCoord &TileCoord) {
  ...
  // Note: VHM-specific API methods will be added once the correct API is
  // determined
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("VHM component created and configured for tile (%d, %d)"),
         TileCoord.X, TileCoord.Y);

  // Configure basic component settings (available on all primitive components)
  // Enable collision so edited terrain affects gameplay collision promptly
  // (Gate B)
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Configured VHM component for tile (%d, %d) with bounds %s"),
         TileCoord.X, TileCoord.Y, *TileBounds.ToString());
}

bool UVHMTerrainRenderer::GenerateMeshFromHeightfield(
    UVirtualHeightfieldMeshComponent *VHMComponent, const FTileCoord &TileCoord,
    UTexture2D *HeightTexture) {
  if (!VHMComponent || !HeightTexture) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("GenerateMeshFromHeightfield - Invalid VHM component or height "
                "texture for tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }

  // Attempt to fetch the cached heightfield so we can build a simple procedural
  // mesh fallback Attempt to fetch the cached heightfield so we can build a
  // simple procedural mesh fallback
  FHeightfieldData CachedHF;
  bool bHasHeightData = false;

  if (HeightfieldService &&
      HeightfieldService->GetCachedHeightfield(TileCoord, CachedHF)) {
    bHasHeightData = true;
  }

  if (!bHasHeightData) {
    // If we don't have height data (e.g. prebaked mode without raw float data),
    // skip proc mesh or use flat? For now, if prebaked mode, we might trust VHM
    // to render. Proc mesh is mostly for collision if VHM collision is partial,
    // or for debug. If VHM works, we can skip this.
    if (VHMSettings.bUsePrebakedHeightfield) {
      // Skip proc mesh generation in prebaked mode if we lack raw data
      // Assuming VHM component handles it or we don't need the fallback
      return true;
    }

    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("GenerateMeshFromHeightfield - Cached heightfield not found "
                "for tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (!Owner) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("GenerateMeshFromHeightfield - VHM component has no owner for "
                "tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (!Proc) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("GenerateMeshFromHeightfield - Failed to create "
                "ProceduralMeshComponent for tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (Resolution <= 1) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("GenerateMeshFromHeightfield - Invalid resolution %d for tile "
                "(%d, %d)"),
           Resolution, TileCoord.X, TileCoord.Y);
    return false;
  }

  const FWorldGenConfig &Config = WorldGenSettings->GetWorldGenConfig();
  const float TileSizeCm =
      Config.TileSizeMeters * 100.0f; // meters -> centimeters
  const float HeightScale =
      Config.HeightfieldScale; // meters -> centimeters (default 100)
  ...
  for (int32 y = 0; y < Resolution; ++y) {
    for (int32 x = 0; x < Resolution; ++x) {
      ...
      const float height = CachedHF.HeightData.IsValidIndex(idx)
                               ? CachedHF.HeightData[idx]
                               : 0.0f;

      Vertices.Add(
          FVector(fx * TileSizeCm, fy * TileSizeCm, height * HeightScale));
  ...
  Proc->CreateMeshSection_LinearColor(0, Vertices, Indices, Normals, UV0,
                                      Colors, Tangents,
                                      /*bCreateCollision*/ true);
  Proc->SetMaterial(
      0, VHMComponent->UPrimitiveComponent::GetMaterial(
             0)); // apply whatever material was set on VHM comp later
  ...
  VHMComponent->SetHiddenInGame(
      true); // hide placeholder VHM component to avoid confusion

  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Procedural terrain mesh created for tile (%d, %d) with %d verts "
              "(TileSize=%.0fcm, HeightScale=%.1f)"),
         TileCoord.X, TileCoord.Y, Vertices.Num(), TileSizeCm, HeightScale);
  return true;
}

bool UVHMTerrainRenderer::ValidateVHMComponent(
    UVirtualHeightfieldMeshComponent *VHMComponent,
    const FTileCoord &TileCoord) const {
  if (!VHMComponent) {
    UE_LOG(
        LogVHMTerrainRenderer, Error,
        TEXT("ValidateVHMComponent - VHM component is null for tile (%d, %d)"),
        TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (!VHMComponent->IsRegistered()) {
    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("ValidateVHMComponent - VHM component is not registered for "
                "tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }
  ...
  if (!Owner || !IsValid(Owner)) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("ValidateVHMComponent - VHM component has invalid owner for "
                "tile (%d, %d)"),
           TileCoord.X, TileCoord.Y);
    return false;
  }

  // Basic validation - VHM-specific validation will be added once the correct
  // API is determined
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("VHM component validation passed for tile (%d, %d)"), TileCoord.X,
         TileCoord.Y);

  UE_LOG(
      LogVHMTerrainRenderer, Log,
      TEXT("ValidateVHMComponent - VHM component is valid for tile (%d, %d)"),
      TileCoord.X, TileCoord.Y);
  return true;
}

FBox UVHMTerrainRenderer::CalculateTileWorldBounds(
    const FTileCoord &TileCoord) const {
  if (!WorldGenSettings) {
  ...
  const float HalfZ = Config.MaxTerrainHeight *
                      Config.HeightfieldScale; // approx top extent in cm
...
void UVHMTerrainRenderer::UpdatePerformanceStats(
    float MeshGenerationTime) const {
...
FVector UVHMTerrainRenderer::GetTileCenterWorldPosition(
    const FTileCoord &TileCoord) const {
  if (!WorldGenSettings) {
  ...
  return FVector(TileCoord.X * TileSizeCm + TileSizeCm * 0.5f,
                 TileCoord.Y * TileSizeCm + TileSizeCm * 0.5f, 0.0f);
}

float UVHMTerrainRenderer::GetLastMeshGenerationTimeMs(
    const FTileCoord &TileCoord) const {
...
FVector UVHMTerrainRenderer::GetTileCornerWorldPosition(
    const FTileCoord &TileCoord) const {
  if (!WorldGenSettings) {
    return FVector::ZeroVector;
  }
  ...
  return FVector(TileCoord.X * TileSizeCm, TileCoord.Y * TileSizeCm, 0.0f);
}

bool UVHMTerrainRenderer::IsValidTileCoordinate(
    const FTileCoord &TileCoord) const {
  // Basic validation - could be expanded based on world limits
  const int32 MaxTileCoordinate = 10000; // Reasonable limit
  return FMath::Abs(TileCoord.X) <= MaxTileCoordinate &&
         FMath::Abs(TileCoord.Y) <= MaxTileCoordinate;
}
  ...
  if (FTerrainMeshData *MeshData = TerrainMeshes.Find(TileCoord)) {
    ...
    if (MeshData->VHMComponent) {
      ...
      if (Owner && IsValid(Owner)) {
        Owner->Destroy();
        UE_LOG(LogVHMTerrainRenderer, Log,
               TEXT("Destroyed VHM actor for tile (%d, %d)"), TileCoord.X,
               TileCoord.Y);
      }
...
bool UVHMTerrainRenderer::InitializeTerrainLODManager() {
  ...
  if (!LODManager) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTerrainLODManager - Failed to create "
                "TerrainLODManager"));
    return false;
  }
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("TerrainLODManager initialized successfully"));
  return true;
}
...
void UVHMTerrainRenderer::UpdateFrameBasedLOD(const FVector &ViewerPosition) {
  ...
  ITerrainLODManager *LODManagerInterface =
      Cast<ITerrainLODManager>(TerrainLODManager.GetObject());
  ...
  for (auto &MeshPair : TerrainMeshes) {
    ...
    if (NewLODLevel >= 0 && NewLODLevel != MeshData.CurrentLODLevel) {
      ...
      if (MeshData.VHMComponent) {
        ...
        UE_LOG(LogVHMTerrainRenderer, VeryVerbose,
               TEXT("Updated LOD for tile (%d, %d) to level %d"), TileCoord.X,
               TileCoord.Y, NewLODLevel);
      }
    }
  }
...
void UVHMTerrainRenderer::HandleVisibilityCulling(
  ...
  for (auto &MeshPair : TerrainMeshes) {
    ...
    if (MeshData.bIsVisible != bShouldBeVisible) {
      ...
      if (MeshData.VHMComponent) {
        ...
        UE_LOG(LogVHMTerrainRenderer, VeryVerbose,
               TEXT("Updated visibility for tile (%d, %d): %s"), TileCoord.X,
               TileCoord.Y,
...
void UVHMTerrainRenderer::ApplyAdaptiveQualityAdjustment() {
  ...
  float CurrentFrameTime =
      FApp::GetDeltaTime() * 1000.0f; // Convert to milliseconds
  ...
  {
    UE_LOG(LogVHMTerrainRenderer, Verbose,
           TEXT("Adaptive quality adjustment: %d LOD transitions, Frame time: "
                "%.2fms"),
...
bool UVHMTerrainRenderer::InitializeHeightfieldTextureManager() {
  UVHMHeightfieldTextureManager *TextureManager =
      NewObject<UVHMHeightfieldTextureManager>(this);
  if (!TextureManager) {
    return false;
  }
  ...
  // Initialize texture manager with VHM settings (call on concrete object, not
  // interface)
...
bool UVHMTerrainRenderer::InitializeTerrainMaterialSystem() {
  if (!BiomeService) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTerrainMaterialSystem - BiomeService is null"));
    return false;
  }

  UVHMTerrainMaterialSystem *MaterialSystem =
      NewObject<UVHMTerrainMaterialSystem>(this);
  if (!MaterialSystem) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTerrainMaterialSystem - Failed to create "
                "TerrainMaterialSystem"));
    return false;
  }
  ...
  // Initialize material system with biome service, VHM settings, and streaming
  // (for water integration)
  if (!MaterialSystem->InitializeWithStreaming(BiomeService, VHMSettings,
                                               TileStreamingService)) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTerrainMaterialSystem - Failed to initialize "
                "TerrainMaterialSystem"));
    return false;
  }

  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("TerrainMaterialSystem initialized successfully"));
  return true;
}

bool UVHMTerrainRenderer::InitializeTileBoundaryManager() {
  UVHMTileBoundaryManager *BoundaryManager =
      NewObject<UVHMTileBoundaryManager>(this);
  if (!BoundaryManager) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTileBoundaryManager - Failed to create "
                "TileBoundaryManager"));
    return false;
  }
  ...
  if (!BoundaryManager->Initialize(HeightfieldService, VHMSettings)) {
    UE_LOG(LogVHMTerrainRenderer, Error,
           TEXT("InitializeTileBoundaryManager - Failed to initialize "
                "TileBoundaryManager"));
    return false;
  }

  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("TileBoundaryManager initialized successfully"));
  return true;
}

bool UVHMTerrainRenderer::UpdateTerrainMeshWithBoundaryStitching(
    const FTileCoord &TileCoord,
    const TArray<FHeightfieldModification> &Modifications,
  ...
  if (bStitchBoundaries && VHMSettings.bEnableBoundaryStitching &&
      TileBoundaryManager.GetInterface()) {
    // Update boundary data for the modified tile
    ITileBoundaryManager *BoundaryManagerInterface =
        Cast<ITileBoundaryManager>(TileBoundaryManager.GetObject());
    if (BoundaryManagerInterface) {
      BoundaryManagerInterface->UpdateTileBoundaryData(
          TileCoord, Modifications, VHMSettings.HeightTextureResolution);
...
TScriptInterface<ITileBoundaryManager>
UVHMTerrainRenderer::GetTileBoundaryManager() const {
  return TileBoundaryManager;
}

TScriptInterface<ITerrainMaterialSystem>
UVHMTerrainRenderer::GetTerrainMaterialSystem() const {
  return TerrainMaterialSystem;
}

bool UVHMTerrainRenderer::ApplyBoundaryStitching(
    const FTileCoord &TileCoord, TArray<float> &InOutHeightData) {
  ...
  for (const FTileCoord &AdjacentTile : AdjacentTiles) {
    ...
    if (TerrainMeshes.Contains(AdjacentTile)) {
      ...
      if (BoundaryManagerInterface->StitchTileBoundaries(
              TileCoord, AdjacentTile, StitchedHeightData,
              VHMSettings.HeightTextureResolution)) {
        InOutHeightData = MoveTemp(StitchedHeightData);
        bAnyStitchingApplied = true;

        UE_LOG(LogVHMTerrainRenderer, Verbose,
               TEXT("Applied boundary stitching between tiles (%d, %d) and "
                    "(%d, %d)"),
...
void UVHMTerrainRenderer::UpdateAdjacentTileBoundaries(
    const FTileCoord &ModifiedTileCoord) {
  if (!TileBoundaryManager.GetInterface()) {
    return;
  }
  ...
  for (const FTileCoord &AdjacentTile : AdjacentTiles) {
    if (FTerrainMeshData *AdjacentMeshData = TerrainMeshes.Find(AdjacentTile)) {
      ...
      if (HeightfieldService && HeightfieldService->GetCachedHeightfield(
                                    AdjacentTile, AdjacentHeightfieldData)) {
        ...
        if (ApplyBoundaryStitching(AdjacentTile, StitchedHeightData)) {
          ...
          if (TextureManagerInterface) {
            ...
            UTexture2D *NewHeightTexture =
                TextureManagerInterface->CreateHeightTexture(
                    AdjacentTile, StitchedHeightData);
            if (NewHeightTexture) {
              AdjacentMeshData->HeightTexture = NewHeightTexture;

              // Update VHM component with new texture
              if (AdjacentMeshData->VHMComponent) {
                GenerateMeshFromHeightfield(AdjacentMeshData->VHMComponent,
                                            AdjacentTile, NewHeightTexture);
              }

              UE_LOG(
                  LogVHMTerrainRenderer, Verbose,
                  TEXT(
                      "Updated adjacent tile (%d, %d) with boundary stitching"),
...
bool UVHMTerrainRenderer::CreateFlatMeadowFallback(
    const FTileCoord &TileCoord) {
  UE_LOG(LogVHMTerrainRenderer, Warning,
         TEXT("Creating flat meadow fallback for tile (%d, %d)"), TileCoord.X,
         TileCoord.Y);
  ...
  float FlatHeight =
      WorldGenSettings ? WorldGenSettings->Settings.SeaLevel : 0.0f;
  ...
  IHeightfieldTextureManager *TextureManagerInterface =
      Cast<IHeightfieldTextureManager>(HeightfieldTextureManager.GetObject());
  UTexture2D *FallbackHeightTexture =
      TextureManagerInterface ? TextureManagerInterface->CreateHeightTexture(
                                    TileCoord, FlatHeightData)
                              : nullptr;

  if (!FallbackHeightTexture) {
    UE_LOG(LogVHMTerrainRenderer, Warning,
           TEXT("Could not create fallback height texture for tile (%d, %d) - "
  ...
  FallbackMeshData.MaterialInstance =
      nullptr; // Will be set below if material system available
  ...
  if (BiomeService && TerrainMaterialSystem.GetInterface()) {
    ...
    if (BiomeService->GetBiomeDefinition(EBiomeType::Meadows, MeadowBiome)) {
      UMaterialInstanceDynamic *FallbackMaterial =
          TerrainMaterialSystem.GetInterface()->CreateTileMaterial(TileCoord,
                                                                   MeadowBiome);
      FallbackMeshData.MaterialInstance = FallbackMaterial;
      UE_LOG(LogVHMTerrainRenderer, Log,
             TEXT("Applied meadow material to fallback tile (%d, %d)"),
             TileCoord.X, TileCoord.Y);
    }
  }
  ...
  UE_LOG(LogVHMTerrainRenderer, Log,
         TEXT("Created flat meadow fallback for tile (%d, %d)"), TileCoord.X,
         TileCoord.Y);
  return true;
}
