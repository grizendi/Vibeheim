// Debug terrain persistence checksum issue
...
void DebugTerrainPersistence()
{
    UE_LOG(LogTemp, Warning, TEXT("=== Debugging Terrain Persistence Checksums ==="));
    ...
    UE_LOG(LogTemp, Warning, TEXT("Step 3: Get modified heightfield and calculate checksum"));
    ...
    // Check loaded modifications count
    ...
    // Regenerate heightfield (should apply loaded modifications)
    ...
    else
    {
        ...
        for (int32 i = 0; i < FMath::Min(ModifiedHeightfield.HeightData.Num(), ReloadedHeightfield.HeightData.Num()); i++)
        {
            ...
            if (Difference > KINDA_SMALL_NUMBER)
            {
                ...
                {
                    UE_LOG(LogTemp, Warning, TEXT("Height difference at index %d: Modified=%.3f, Reloaded=%.3f, Diff=%.3f"), 
                        i, ModifiedHeight, ReloadedHeight, Difference);
                }
            }
        }
        
        UE_LOG(LogTemp, Warning, TEXT("Total height differences: %d, Max difference: %.3f"), 
            DifferenceCount, MaxDifference);
    }
    
    UE_LOG(LogTemp, Warning, TEXT("=== Terrain Persistence Debug Complete ==="));
}