...
## 0. Test Diet (One-time Cleanup)
- Disable all but four smoke tests: Determinism, Integration, Persistence, VHM-Smoke
- Remove long-running/stress/duplicate test suites to keep compile time and mental load low
- Keep only essential validation tests for core functionality
...
- [ ] 2. Map + Render + Seams + Fixed Ring
  - Create /Game/Maps/WG_TestMap.umap with single AWorldGenManager (spawn if missing)
  - Pick one seam strategy and ship it: either small skirt or index stitching (not both)
  - Normalize normals/tangents across tile borders to prevent lighting seams
  - Create fixed streaming ring (Generate=9, Load=5, Active=3) with tiny hysteresis (keep tiles alive 1 ring beyond Active)
  - Ensure PCG spawns only after height+biome+material ID are Ready (not just height)
  - Add flat meadow fallback for failures with one error code line
  - **Gate A**: Orbit seam at two LOD thresholds - no gaps, no lighting steps, no foliage popping at blend bands
  - _Requirements: 1.1, 1.2, 1.3, 1.6_

- [ ] 3. Terrain Edits + Persistence Round-Trip
  - Implement 4 brushes: raise/lower/smooth/noise with radius & falloff parameters
  ...
  - Persist edits to Saved/Vibeheim/WorldGen/Mods/ with versioned journal (warn once on mismatch, skip cleanly)
  - Verify terrain edits reload correctly after restart
...
- [ ] 6. Final Polish & Definition of Done
